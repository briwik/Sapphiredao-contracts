...
contract SapTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SSAP }
    ...
    address public sSAP;
    uint public sSAPQueue; // Delays change to sSAP address
    ...
    /**
        @notice allow approved address to deposit an asset for SAP
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint SAP needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( SAP ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn SAP for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ISAPERC20( SAP ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sSAP ).balanceOf( msg.sender ); // Can only borrow against sSAP held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with SAP
        @param _amount uint
     */
    function repayDebtWithSAP( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISAPERC20( SAP ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, SAP, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( SAP ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( SAP ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns SAP valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match SAP decimals
            value_ = _amount.mul( 10 ** IERC20( SAP ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSAP ) { // 9
            sSAPQueue = block.number.add( blocksNeededForQueue );
        } else return false;
